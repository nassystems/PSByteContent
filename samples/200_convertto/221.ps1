& {
# 3 バイトごとに "-" を打つ。
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLength 3 -Separator '-'
Assert ($hexstring -ceq '081329-39425b-6e7b8b-97a2b7-cadfe5-f3051b-243644-586c7b-8f9ea3-bcc3db-e7fa')
}
& {
# 区切り文字は大文字化されない。
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLength 3 -Separator 'x' -Capital
...
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLengths 3, 16, 48 -Separators '-', '__', '**'
...
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLengths 4, 16 -Separators '-', '_'
Assert ($hexstring -ceq '08132939-425b6e7b-8b97a2b7-cadfe5f3_051b2436-44586c7b-8f9ea3bc-c3dbe7fa')
}
& {
# 1 バイトごとにスペースを打つ。
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLength 1 -Separator ' '
Assert ($hexstring -ceq '08 13 29 39 42 5b 6e 7b 8b 97 a2 b7 ca df e5 f3 05 1b 24 36 44 58 6c 7b 8f 9e a3 bc c3 db e7 fa')
}
& {
# 4 バイトごとに "-" を打つ。
# 8 バイトごとに文字列を分割する。
$hexstring = ConvertFrom-ByteContent -InputObject $samplebytelist[1] -BlockLengths 4, 8 -Separators '-'
